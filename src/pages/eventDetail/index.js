...
const EventDetail = () => {
  ...
  const handleBuyTicket = () => {
    history.push(`/${Number(register_type) === 1 ? 'exhibitor' : 'visitor'}/buy-ticket`, { key: key, price: state.data.visitors_package_price });
  }
          ...
          <div className="col-sm-12 pt-3 pb-3">
            <div className="detailContainer text-wrap shadow-sm">
              ...
              <div className="p-3">
                ...
                {buy === false ? null : state.data.pending_stall_booking.length > 0 ? <button className="btn btn-primary" onClick={handleContinueBuyTicket}>Continue Stall Booking</button> : <button className="btn btn-primary" onClick={handleBuyTicket}>{Number(register_type) === 1 ? 'Book Stall' : 'Buy Entry Pass'}</button>}
              </div>
            </div>
          </div>
        </div>
